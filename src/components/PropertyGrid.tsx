    ...
    <div className="w-full bg-slate-950 px-6 py-12 md:py-16 md:px-12 max-w-7xl mx-auto font-sans" id="property-grid-module">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between mb-10 border-b border-slate-900 pb-6" id="grid-header">
        <div>
          ...
          <h2 className="text-3xl font-serif text-slate-100">Select Your Sanctuary</h2>
        </div>

        {/* Mobile Filter Toggle & Quick Info */}
        <div className="flex items-center gap-4 mt-4 md:mt-0">
          <p className="text-xs text-slate-400 font-mono">
            Showing <span className="text-slate-100 font-medium">{properties.length}</span> exceptional property matches
          </p>
          <button
            onClick={() => setShowMobileFilters(true)}
            className="md:hidden flex items-center gap-2 bg-slate-900 border border-slate-800 text-slate-100 px-4 py-2.5 rounded-xl text-xs font-medium uppercase tracking-wider hover:bg-slate-800 transition-colors"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-8 items-start">
        ...
        <div className="hidden md:block col-span-1 sticky top-28 bg-slate-900/40 border border-slate-900 rounded-2xl p-6" id="desktop-filters">
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-xs font-mono font-semibold uppercase tracking-widest text-slate-100">Refine Search</h3>
          ...
          <div className="mb-8">
            <div className="flex justify-between items-center mb-3">
              ...
              <span className="text-xs text-amber-500 font-mono font-semibold">ETB {filters.priceRange[1].toLocaleString()}</span>
            </div>
            <input
              type="range"
              min="500"
          ...
          <div className="mb-8">
            ...
            <div className="space-y-2.5">
              {ALL_AMENITIES.map((amenity) => {
                ...
                return (
                  <label key={amenity} className="flex items-center gap-3 cursor-pointer group text-xs text-slate-400 hover:text-slate-100 transition-colors">
                    <div
                      onClick={() => handleAmenityToggle(amenity)}
                      className={`w-4 h-4 rounded border flex items-center justify-center transition-all duration-150 ${checked ? "bg-amber-500 border-amber-500" : "border-slate-700 bg-slate-950"
                        }`}
                    >
              ...
              {[5, 4].map((star) => {
                ...
                return (
                  <button
                    ...
                    className={`py-2 px-3 rounded-lg border text-xs font-mono flex items-center justify-center gap-1.5 transition-all duration-200 ${checked
                  ...
                  >
                    ...
                    <div className="relative aspect-[4/3] w-full overflow-hidden bg-slate-950">
                      ...
                      >
                        <Heart
                          size={15}
                          className={`transition-transform duration-300 active:scale-125 ${isFavorite ? "fill-amber-500 text-amber-500" : "text-white"
                            }`}
                        />
                      </button>
                      ...
                      <div className="absolute bottom-4 inset-x-0 flex justify-center gap-1.5 z-10">
                        {property.images.map((_, idx) => (
                          <div
                            key={idx}
                            className={`h-1 rounded-full transition-all duration-300 ${idx === activeImgIdx ? "w-4 bg-amber-500" : "w-1.5 bg-white/45"
                              }`}
                          />
                        ))}
                      </div>
                      ...
                      <div className="absolute bottom-4 right-4 z-10 bg-slate-950/85 backdrop-blur-md px-3 py-1.5 rounded-lg border border-slate-800/80 text-xs font-mono text-slate-100">
                        From <span className="font-semibold text-amber-500">ETB {property.basePrice.toLocaleString()}</span> / night
                      </div>
                    </div>
                    ...
                    <div className="p-6 flex flex-col flex-grow">
                      ...
                      <h3 className="font-serif text-lg text-slate-100 mb-1 tracking-tight group-hover:text-amber-500 transition-colors">
                        {property.name}
                      </h3>
          ...
          <>
            ...
            >
              <div className="flex items-center justify-between mb-6 border-b border-slate-800 pb-4">
                ...
                <button
                  onClick={() => setShowMobileFilters(false)}
                  className="w-8 h-8 rounded-full bg-slate-950 border border-slate-800 flex items-center justify-center text-slate-400 hover:text-slate-100"
                >
                  <X size={14} />
                </button>
              </div>
              ...
              <div className="mb-8">
                ...
                <div className="space-y-3">
                  {ALL_AMENITIES.map((amenity) => {
                    const checked = filters.amenities.includes(amenity);
                    return (
                      <label key={amenity} className="flex items-center gap-3 cursor-pointer text-xs text-slate-400 hover:text-slate-100">
                        <div
                          onClick={() => handleAmenityToggle(amenity)}
                          className={`w-4 h-4 rounded border flex items-center justify-center transition-all ${checked ? "bg-amber-500 border-amber-500" : "border-slate-700 bg-slate-950"
                            }`}
                        >
                          {checked && <Check size={10} className="text-slate-950 stroke-[3]" />}
                        </div>
                  ...
                  {[5, 4].map((star) => {
                    ...
                    return (
                      <button
                        ...
                        type="button"
                        onClick={() => handleStarToggle(star)}
                        className={`py-2 px-3 rounded-lg border text-xs font-mono flex items-center justify-center gap-1.5 transition-all ${checked
                          ? "bg-amber-500/10 border-amber-500 text-amber-500"
