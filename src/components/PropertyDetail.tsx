    ...
    <div className="bg-slate-950 text-slate-100 min-h-screen py-6 px-4 md:px-12 max-w-7xl mx-auto font-sans" id="property-detail-module">
      ...
      <div className="flex items-center justify-between mb-8" id="detail-nav-bar">
        ...
        <div className="flex items-center gap-4">
          ...
          <button
            ...
            className="flex items-center gap-1.5 text-xs font-mono text-slate-400 hover:text-slate-100 transition-colors"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
        <div className="lg:col-span-8 space-y-12">
          ...
          <div id="detail-description">
            ...
            <h1 className="text-4xl md:text-5xl font-serif text-slate-100 tracking-tight leading-tight mb-4">
              {property.name}
            </h1>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-8 pt-6 border-t border-slate-900">
              <div className="p-4 bg-slate-900/30 border border-slate-900 rounded-xl flex flex-col justify-between">
                ...
                <span className="text-sm font-medium text-slate-100 flex items-center gap-1 font-mono">
          ...
          <div id="room-selection-matrix">
            <div className="flex items-center justify-between mb-6 pb-3 border-b border-slate-900">
              <h3 className="text-lg font-serif text-slate-100 tracking-tight flex items-center gap-2">
                  ...
                  >
                    <div className="flex-1 space-y-3">
                      <div className="flex items-center gap-3">
                        <h4 className="text-base font-serif text-slate-100 font-medium">{room.name}</h4>
          ...
          <div id="property-amenities">
            <h3 className="text-lg font-serif text-slate-100 tracking-tight mb-4 pb-3 border-b border-slate-900">
          ...
          <div id="reviews-section" className="space-y-6">
            <h3 className="text-lg font-serif text-slate-100 tracking-tight pb-3 border-b border-slate-900">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-10 gap-y-4 p-6 bg-slate-900/20 border border-slate-900/70 rounded-2xl">
              <div>
                <div className="flex justify-between text-xs font-mono mb-1.5">
                  ...
                  <span className="text-slate-100 font-medium">{sentiment.service} / 5.0</span>
                </div>
              ...
              <div>
                <div className="flex justify-between text-xs font-mono mb-1.5">
                  ...
                  <span className="text-slate-100 font-medium">{sentiment.design} / 5.0</span>
                </div>
                <div className="h-1 bg-slate-850 rounded-full overflow-hidden">
              ...
              <div>
                <div className="flex justify-between text-xs font-mono mb-1.5">
                  ...
                  <span className="text-slate-100 font-medium">{sentiment.location} / 5.0</span>
                </div>
                <div className="h-1 bg-slate-850 rounded-full overflow-hidden">
              ...
              <div>
                <div className="flex justify-between text-xs font-mono mb-1.5">
                  ...
                  <span className="text-slate-100 font-medium">{sentiment.wellness} / 5.0</span>
                </div>
                <div className="h-1 bg-slate-850 rounded-full overflow-hidden">
              ...
              {property.reviews.map((review) => (
                <div key={review.id} className="p-6 bg-slate-900/10 border border-slate-900 rounded-xl space-y-3">
                  <div className="flex items-center justify-between">
                    <div>
                      <h4 className="text-sm font-medium text-slate-100">{review.author}</h4>
        ...
        <div className="lg:col-span-4 sticky top-28 bg-slate-900/60 backdrop-blur-xl border border-slate-800 rounded-2xl p-6 shadow-2xl" id="sticky-booking-widget">
          ...
          <div className="border-b border-slate-800/80 pb-4 mb-4">
            ...
            <div className="text-base font-serif text-slate-100 mt-1">{selectedRoom.name}</div>
            <div className="text-xs text-slate-400 font-mono mt-0.5">ETB {selectedRoom.pricePerNight.toLocaleString()} / night</div>
          </div>
          ...
          <div className="space-y-3 mb-6">
            <div className="flex justify-between items-center text-xs">
              ...
              <span className="text-slate-100 font-mono">{formatDate(searchQuery.startDate, "June 15, 2026")}</span>
            </div>
            ...
            <div className="flex justify-between items-center text-xs">
              ...
              <span className="text-slate-100 font-mono">{formatDate(searchQuery.endDate, "June 18, 2026")}</span>
            </div>
          ...
          <div className="border-t border-b border-slate-850 py-4 mb-6 space-y-2.5 text-xs font-mono">
            <div className="flex justify-between text-slate-400">
              <span>ETB {selectedRoom.pricePerNight.toLocaleString()} × {nights} nights</span>
              <span className="text-slate-100">ETB {subtotal.toLocaleString()}</span>
            </div>
            <div className="flex justify-between text-slate-400">
              <span>Luxury Resort Fee (8%)</span>
              <span className="text-slate-100">ETB {resortFee.toLocaleString()}</span>
            </div>
            <div className="flex justify-between text-slate-400">
              <span>Local eco-tax (10%)</span>
              <span className="text-slate-100">ETB {localTax.toLocaleString()}</span>
            </div>
            <div className="flex justify-between text-slate-100 font-semibold pt-2 border-t border-slate-850 text-sm">
      ...
      {lightboxImage && (
        ...
        >
          ...
          <button 
            onClick={() => setLightboxImage(null)}
            className="absolute top-6 right-6 w-10 h-10 rounded-full bg-slate-900 border border-slate-800 text-slate-100 flex items-center justify-center"
          >
            <ArrowLeft size={16} />
          </button>
        </div>
